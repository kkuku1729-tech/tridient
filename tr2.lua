-- ExpensiveMods для Trident Survival - ИСПРАВЛЕННАЯ ВЕРСИЯ
...
-- Слайдер скорости полета - ИСПРАВЛЕННЫЙ
...
-- Логика переключателя Fly - ИСПРАВЛЕННАЯ
...
UserInputService.InputChanged:Connect(function(input)
    if SliderDragging and input.UserInputType == Enum.UserInputType.MouseMovement then
        ...
        FlySpeed = math.floor(1 + relativeX * 99) -- От 1 до 100
        SpeedLabel.Text = "Fly Speed: " .. FlySpeed
        SliderThumb.Position = UDim2.new(relativeX, -10, 0, 25)
    end
end)

-- Обработка полета - ИСПРАВЛЕННАЯ
local FlyConnection
FlyConnection = RunService.Heartbeat:Connect(function()
    if FlyEnabled and BodyVelocity and LocalPlayer.Character and LocalPlayer.Character.HumanoidRootPart then
        local newVelocity = Vector3.new(0, 0, 0)
        
        if UserInputService:IsKeyDown(Enum.KeyCode.W) then
            newVelocity = newVelocity + (LocalPlayer.Character.HumanoidRootPart.CFrame.LookVector * FlySpeed)
        end
        if UserInputService:IsKeyDown(Enum.KeyCode.S) then
            newVelocity = newVelocity + (LocalPlayer.Character.HumanoidRootPart.CFrame.LookVector * -FlySpeed)
        end
        if UserInputService:IsKeyDown(Enum.KeyCode.A) then
            newVelocity = newVelocity + (LocalPlayer.Character.HumanoidRootPart.CFrame.RightVector * -FlySpeed)
        end
